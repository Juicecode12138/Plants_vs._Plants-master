...
IMAGE img_1P;										//1P 文字图片
IMAGE img_2P;										//2P 文字图片
IMAGE img_1P_desc;									//1P 键位描述图片
IMAGE img_2P_desc;									//2P 键位描述图片
IMAGE img_gravestone_left;							//朝向左的墓碑照片
IMAGE img_gravestone_right;							//朝向右的墓碑照片
...
IMAGE img_avator_peashooter;						//豌豆射手头像图片
...
{
	dst.clear();//清空目标图集中已有的图片内容
	for (int i = 0; i < src.get_size(); i++) //对原图集中的每一帧图片都执行flip_image操作
	{
		IMAGE img_flpipped;
